...
extern long long Ntype[7];      /*!< total number of particles of each type */
extern int NtypeLocal[7];       /*!< local number of particles of each type */
...
extern struct global_data_all_processes
{
  ...
  double SofteningSink;         /*!< comoving gravitational softening lengths for type 6 */ 
...
extern struct state_of_system
{
  ...
  double MassComp[7];
  double EnergyKinComp[7];
  double EnergyPotComp[7];
  double EnergyIntComp[7];
  double EnergyTotComp[7];
  double MomentumComp[7][4]; 
  double AngMomentumComp[7][4]; 
  double CenterOfMassComp[7][4];
}
