...
void find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */ )
{
  int i, j, type, *temp;
  int count[7];
  long long count_sum[7];
  double v[7], v_sum[7], mim[7], min_mass[7];
  ...
  if(All.ComovingIntegrationOn)
      ...
      for(i = 0; i < NumPart; i++)
	{
      ...
      MPI_Allreduce(v, v_sum, 7, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
      MPI_Allreduce(mim, min_mass, 7, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);

      temp = malloc(NTask * 7 * sizeof(int));
      MPI_Allgather(count, 7, MPI_INT, temp, 7, MPI_INT, MPI_COMM_WORLD);
      for(i = 0; i < 7; i++)
	{
	  count_sum[i] = 0;
	  for(j = 0; j < NTask; j++)
	    count_sum[i] += temp[j * 7 + i];
	}
      free(temp);
